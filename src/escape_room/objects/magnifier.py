...
class Magnifier:
    def __init__(self, inventory,room_state,unique_id="",shift_coordinates=(0,0,0),room_placement=False):
        ...
        self.image_path = ContextManager.get_image_path().joinpath("magnifier.png")
    ...
    def draw(self, canvas):
        ...
        if self.inventory.objectInInventory("magnifier",self.object_owner):
            objIndex = self.inventory.getObjectIndex("magnifier",self.object_owner)
        ...
        if self.inventory.objectIsSelected("magnifier",self.object_owner):
            objIndex = self.inventory.getObjectIndex("magnifier",self.object_owner)
            (x1,y1) = self.inventory.getObjectCoordinates(objIndex)
    ...
    def on_key_click(self, event, tooltip_data):
        ...
        if not self.inventory.objectInInventory("magnifier",self.object_owner) and \
               self.room_placement == True:
            self.inventory.addObject("magnifier",self.object_owner,self)
            self.room_state.remove("magnifier",self.unique_id)
            self.room_placement = False
        else:
            self.inventory.selectObject("magnifier",self.object_owner)
